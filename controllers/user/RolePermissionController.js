...
self.get = async(req, res) => {
    try {
        ...
        let data = await rolepermission.findOne({
            where: {
                id: id
            }
        });
...
self.save = async(req, res) => {
    try {
        let body = req.body;
        if (usr) {
            let data = await rolepermission.create(body);
            return res.json(data)
        }
    } catch (error) {
        res.status(500).json({
...
self.update = async(req, res) => {
    try {
        let id = req.params.id;
        let body = req.body;
        let data = await rolepermission.update(body, {
            where: {
                id: id
            }
        });
...
self.delete = async(req, res) => {
    try {
        let id = req.params.id;
        let data = await rolepermission.destroy({
            where: {
                id: id
            }
        });
